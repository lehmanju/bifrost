...
4. Enter a comma-separated list of header names or `Name=value` pairs (e.g., `X-Tenant-ID, X-Proxy-Token=secret`)
...
|-------|------|----------|-------------|
| `required_headers` | `string[]` | No | List of header names (or `Name=value` pairs) that must be present on every request. Name matching is case-insensitive; value matching is case-sensitive. |
...
Validate that the caller provides an exact token value:
